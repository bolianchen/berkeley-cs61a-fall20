def couple(s, t):
    ...
    assert len(s) == len(t)
    "*** YOUR CODE HERE ***"
    return [[e1, e2] for e1, e2 in zip(s,t)]
...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if isinstance(label(t), str) and 'berry' in label(t):
        return True
    for b in branches(t):
        if berry_finder(b):
            return True
...
def add_trees(t1, t2):
    ...
    if len(bs1)>len(bs2):
        new_branches.extend(bs1[len(bs2):])
    if len(bs2)>len(bs1):
        new_branches.extend(bs2[len(bs1):])
...
if __name__ == '__main__':
    import random
    random.seed(1234)
    l1 = [random.randint(0,10) for _ in range(1000)]
